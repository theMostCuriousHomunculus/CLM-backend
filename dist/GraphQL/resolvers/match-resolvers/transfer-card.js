...
export default async function (parent, args, context) {
    ...
    match.log.push(`${account.name} moved ${match.players.every((plr) => card.visibility.includes(plr.account))
        ? card.name
        : 'a card'} from ${originZone.toString() === 'stack' ? 'the stack' : 'their ' + originZone} to ${destinationZone.toString() === 'stack'
        ? 'the stack'
        : 'their ' + destinationZone}.`);
    if (originZone.toString() === 'library')
        card.face_down = true;
    if (reveal) {
        for (const plr of match.players) {
            if (!card.visibility.includes(plr.account))
                card.visibility.push(plr.account);
        }
    }
    ...
    if (destinationZone.toString() === 'library' && !shuffle) {
        ...
        player.library = player.library
            .slice(0, index)
            .concat([card])
            .concat(player.library.slice(index));
    }
    else if (destinationZone.toString() === 'library' &&
        shuffle &&
        typeof index === 'number') {
        ...
        if (destinationZone.toString() === 'hand' &&
            !card.visibility.includes(account._id))
