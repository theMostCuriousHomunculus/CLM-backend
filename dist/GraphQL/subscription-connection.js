...
export default async function (context) {
    ...
    if (context.connectionParams.blogPostID) {
        ...
        if (!blogPost)
            throw new Error('Could not find a blog post with the provided blogPostID.');
    ...
    if (context.connectionParams.eventID) {
        ...
        if (!event)
            throw new Error('An event with that ID does not exist or you were not invited to it.');
        context.event = event;
    }
