...
export default async function (req, res, next) {
    try {
        ...
        if (req.header('EventID')) {
            ...
            req.event = event;
            if (event) {
                req.player = event.players.find((plr) => plr.account.toString() === req.account._id.toString());
            }
        }
        if (req.header('MatchID')) {
            ...
            if (match && req.account) {
                req.player = match.players.find((plr) => plr.account.toString() === req.account._id.toString());
            }
        }
        req.pubsub = pubsub;
    }
